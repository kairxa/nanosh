...
func TestPowerCache(t *testing.T) {
	t.Run("should add 4-8 eCells", func(t *testing.T) {
		...
		result, err := PowerCache(game, gameID, invokeTime, username)
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}

		// Check eCells increased by 4-8
		gain := result.Ship.ECells - initialECells
		if gain < 4 || gain > 8 {
			t.Errorf("Expected eCells to increase by 4-8, gained %d", gain)
		}
	})
	...
	t.Run("should return error when no expedition active", func(t *testing.T) {
		...
		_, err = PowerCache(game, gameID, invokeTime, username)
	...
	t.Run("should be deterministic with same seed", func(t *testing.T) {
		...
		result1, _ := PowerCache(game1, gameID, invokeTime, username)
		result2, _ := PowerCache(game2, gameID, invokeTime, username)
	...
	t.Run("should have varied results across range", func(t *testing.T) {
		...
		for i := range 40 {
			...
			result, _ := PowerCache(game, gameID, int64(i*1000), username)
			gain := result.Ship.ECells - initialECells
		...
		// Should see reasonable spread across 4-8 range
		if minSeen > 5 || maxSeen < 7 {
