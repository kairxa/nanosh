...
func TestStillUsableECells(t *testing.T) {
	t.Run("should add 6-12 eCells", func(t *testing.T) {
		...
		result, err := StillUsableECells(game, gameID, invokeTime, username)
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}

		// Check eCells increased by 6-12
		eCellsGain := result.Ship.ECells - initialECells
		if eCellsGain < 6 || eCellsGain > 12 {
			t.Errorf("Expected eCells gain between 6-12, got %d", eCellsGain)
		}
	})
	...
	t.Run("should return error when no expedition active", func(t *testing.T) {
		...
		_, err = StillUsableECells(game, gameID, invokeTime, username)
	...
	t.Run("should be deterministic with same seed", func(t *testing.T) {
		...
		result1, _ := StillUsableECells(game1, gameID, invokeTime, username)
		result2, _ := StillUsableECells(game2, gameID, invokeTime, username)
	...
	t.Run("should have varied results across range", func(t *testing.T) {
		...
		for i := 0; i < 50; i++ {
			...
			result, _ := StillUsableECells(game, gameID, invokeTime, username)
			eCellsGain := result.Ship.ECells - initialECells
		...
		for gain := range eCellsResults {
			if gain < 6 || gain > 12 {
				t.Errorf("ECells gain %d outside expected range 6-12", gain)
			}
		}
	})
	...
	t.Run("should stack with existing eCells", func(t *testing.T) {
		...
		result, _ := StillUsableECells(game, "test", int64(1000), "user")
		...
		if gain < 6 || gain > 12 {
			t.Errorf("Expected gain 6-12, got %d", gain)
		}
	})
}