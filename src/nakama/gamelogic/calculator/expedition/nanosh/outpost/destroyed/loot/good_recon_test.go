...
func TestGoodRecon(t *testing.T) {
	t.Run("should add 1 Basic Intel", func(t *testing.T) {
		...
		result, err := GoodRecon(game, gameID, invokeTime, username)
		if err != nil {
	...
	t.Run("should return error when no expedition active", func(t *testing.T) {
		...
		_, err = GoodRecon(game, gameID, invokeTime, username)
	...
	t.Run("should be deterministic (no randomness)", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			...
			result, _ := GoodRecon(game, gameID, invokeTime, username)
