...
describe('action.weapon-system.calibrate', () => {
  ...
  it('should optimize weapon', () => {
    ...
    expect(
      newState?.ship.modifiers.get('ship.persistent.weapon.optimized'),
    ).toMatchObject({
      start: { day: 1, cycle: 1 },
    ...
    ).toBe('action.weapon-system.calibrate')
    expect(
      newState?.characters
        .get('Soren Koda')
  ...
  it('should optimize weapon with another character non dutiful and non diligent', () => {
    const [newState, error] = calibrate({
      state: gameState!,
      gameID: 'calibrate-test',
      invokeTime: 123,
