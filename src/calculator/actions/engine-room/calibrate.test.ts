...
describe('action.engine-room.calibrate', () => {
  ...
  it('should optimize engine', () => {
    ...
    expect(
      newState?.ship.modifiers.get('ship.persistent.engine.optimized'),
    ).toMatchObject({
      start: { day: 1, cycle: 1 },
    ...
    ).toBe('action.engine-room.calibrate')
    expect(
      newState?.characters
        .get('Soren Koda')
  ...
  it('should optimize engine with another character non dutiful and non diligent', () => {
    const [newState, error] = calibrate({
      state: gameState!,
      gameID: 'calibrate-test',
      invokeTime: 123,
